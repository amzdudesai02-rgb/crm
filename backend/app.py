...
frontend_origins = [
    ...
    "https://leveragecrm.amzdudes.io",     # Production frontend (Hostinger - old)
    "https://crm.amzdudes.io",             # Production frontend (Hostinger - new)
]
...
def register_user(form: dict, db: Session = Depends(get_db)):
    ...
    if existing_user:
        # Special message for demo account
        if form["email"] == "no-reply@amzdudes.io":
            raise HTTPException(
                status_code=400, 
                detail="Demo account already exists. Please login instead."
            )
...
def seed_demo_data_for_user(user: User, db: Session):
    """Automatically seed demo data for demo account"""
    DEMO_EMAIL = "no-reply@amzdudes.io"
    
    # Only seed for demo email
    if user.email != DEMO_EMAIL:
        return
    
    # Check if data already exists (avoid duplicate seeding)
    existing_companies = db.query(Company).filter(Company.created_by == user.id).count()
    ...
    try:
        ...
        for idx, stage_name in enumerate(stage_names):
            ...
            if not stage:
                stage = PipelineStage(id=uuid4(), name=stage_name, order_index=idx, created_at=datetime.utcnow())
                db.add(stage)
                db.commit()
        ...
        for i, po_data in enumerate(PURCHASE_ORDERS):
            if i < len(deals_list):
                ...
                po = PurchaseOrder(
                    ...
                    cogs_total=po_data["total_amount"] * 0.7,
        ...
        # Create shipments
        pos = db.query(PurchaseOrder).filter(PurchaseOrder.created_by == user.id).limit(2).all()
        SHIPMENTS = [
        ...
        for i, inv_data in enumerate(INVOICES):
            status_map = {"paid": "paid", "pending": "issued"}
            invoice = Invoice(
                id=uuid4(),
        ...
        REMINDERS = [
            {"title": "Follow up with TechBrand - Send pricing proposal", "due_date_days": 2},
            {"title": "Review Global Suppliers contract - Legal review needed", "due_date_days": 4},
            {"title": "Call Premium Products - Discuss MOQ requirements", "due_date_days": 1},
        ]
        ...
        print(f"✅ Auto-seeded demo data for {user.email}")
    except Exception as e:
        db.rollback()
        print(f"⚠️ Failed to seed demo data: {e}")
        # Don't fail login if seeding fails
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    # Auto-seed demo data for demo account
    seed_demo_data_for_user(user, db)
